...
class AdminSignupSerializer(serializers.ModelSerializer):
    class Meta:
        model = Admin
        fields = ['nom_complet', 'email', 'telephone', 'password', 'lienPhoto']
        extra_kwargs = {'password': {'write_only': True}}

class PersonnelMedicalSignupSerializer(serializers.ModelSerializer):
    class Meta:
        model = PersonnelMedical
        fields = ['nom_complet', 'email', 'specialite', 'telephone', 'password', 'role', 'lienPhoto']
        extra_kwargs = {'password': {'write_only': True}}

class PatientSignupSerializer(serializers.ModelSerializer):
    class Meta:
        model = Patient
        fields = ['nss', 'nom_complet', 'date_naissance', 'adresse', 'telephone', 'email', 
