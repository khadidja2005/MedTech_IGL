...
def update_etablissement(request, etablissement_id):
    try:
        etablissement = get_object_or_404(Etablissement, id=etablissement_id)
        etablissement.nom_etablissement = request.data.get('nom_etablissement', etablissement.nom_etablissement)
...
def get_personnel_medical_by_etablissement(request, etablissement_id):
    try:
        personnel_medical = (PersonnelMedical.objects
                           .filter(etablissement_personnel_medical__etablissement_id=etablissement_id)
...
def update_personnel_medical_in_etablissement(request, etablissement_id, personnel_id):
    try:
        personnel = get_object_or_404(PersonnelMedical, 
                                    id=personnel_id,
...
@api_view(['DELETE'])
def delete_personnel_medical_from_etablissement(request, etablissement_id, personnel_id):
    try:
...
def get_dpis_by_etablissement(request, etablissement_id):
    try:
        dpis = (DPI.objects
                .filter(patient__antecedent__patient__mutuelle__patient__contact__patient__id__in=
...
def delete_dpi(request, dpi_id):
    try:
        ...
        return Response({
            "status": "success",
            "message": "DPI supprimé avec succès"
        })
    except Exception as e:
        return Response({
            "status": "error",
